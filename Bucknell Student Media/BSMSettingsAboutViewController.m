...
- (void)viewDidLoad
{
    ...
    // Read in RTF file with acknowledgements. We shouldn't need to change this more often than every release of the app, so it shouldn't be necessary to fetch a copy from the network.
    ...
    _aboutTextView.attributedText = [self updateUserFontSizeForAttributedString:attributedString withTextStyle:UIFontTextStyleBody];
}
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(updateAboutTextViewFont)
                                                 name:UIApplicationDidBecomeActiveNotification object:nil];
}

/**
 *  Updates the aboutTextView font to reflect the latest Dynamic Type settings within Settings.app. This is really only useful in the off chance that a user changes their Dynamic Type setting while the app is running, but stranger things have happened!
 */
- (void)updateAboutTextViewFont {
    _aboutTextView.attributedText = [self updateUserFontSizeForAttributedString:_aboutTextView.attributedText
